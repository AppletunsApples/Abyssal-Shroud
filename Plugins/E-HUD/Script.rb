...
class HUD
  ...
  BACKGROUND_COLOR = Color.new(211, 211, 211)
  ...
  def textsDefined
    ret=[]
    ret[0] = _INTL("{1}", $player.name)
      level_cap = Story_LevelStats.get_story_level
      ret[1] = _INTL("Story Lv. {1}", level_cap.to_s)
      return ret
  end
  ...
  def createTonePerStatus
    ...
    ret[:FROSTBITE]    = Tone.new(153,204,204,50)
    # For compatibility with older versions
    statusArray = [:NONE, :SLEEP, :POISON, :BURN, :PARALYSIS, :FROSTBITE]
  ...
  def createPokemon(i)
    ...
    spacing = 64
    right_offset = 16
    @sprites["pokeicon#{i}"].x = Graphics.width - right_offset - spacing * (6 - i)
    @sprites["pokeicon#{i}"].y = y
    createPokemonHPBar(i, @sprites["pokeicon#{i}"].x + 32, @yposition + 55, 36, 10)
  end
