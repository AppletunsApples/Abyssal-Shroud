...
class PokemonLoadScreen
  ...
  def prompt_save_deletion(file_path)
    ...
    self.delete_save_data(file_path) if pbConfirmMessage(
    ...
    loop do
      ...
      if index >= 0
        ...
        if !File.file?(SaveData.get_full_path(slot)) ||
            pbConfirmMessage(_INTL("Are you sure you want to overwrite the save in #{slot}?")) # If the slot names were changed this grammar might need adjustment.
          pbSEPlay('GUI save choice')
          ret = doSave(slot)
        end
      elsif exiting # Pressed cancel
        next unless pbConfirmMessage(_INTL("Are you sure you want to quit without saving?"))
      end
      break
    end
